...
export default function AlojamentosScreen() {
  ...
  useEffect(() => {
    const sortedData = [...alojamentosData].sort((a, b) => a.nome.localeCompare(b.nome));
    setAlojamentos(sortedData);
    setLoading(false);
  }, []);

  const renderItem = ({ item }) => (
    <Link href={{ pathname: "/alojamento/[id]", params: { id: item.id } }} asChild>
      <TouchableOpacity style={styles.itemContainer}>
        <Ionicons name="bed-outline" size={32} color="#3B82F6" style={styles.icon} />
